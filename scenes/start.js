class Start extends Phaser.Scene {
    constructor() {
        ...
        this.LAYOUT_PORTRAIT = {
            ...
            download_button: { x: 541, y: 1337, scale: 0.081, depth: 100 },
            ...
            fighter_15: { x: 797, y: 908, scale: 0.029, angle: 77.349, depth: 5 },
        ...
        this.LAYOUT_LANDSCAPE = {
            ...
            new_bg: { x: 962, y: 790, scale: 1.982, depth: 1 },
            counting_machine: { x: 349, y: 1244, scale: 0.237, depth: 100 },
            ...
            download_button: { x: 1208, y: 1340, scale: 0.14, depth: 100 },
            ...
            iq_counter: { x: 2088, y: 1240, scale: 0.115, depth: 100 },
    ...
    createStartOverlay() {
        ...
        this.startOverlay = this.add.rectangle(0, 0, 1, 1, 0x000000, 0.72)
            .setOrigin(0)
            .setDepth(1000);
        ...
        this.input.once('pointerdown', () => this.transitionToGame());
    }

    updateStartOverlayLayout() {
        if (!this.startOverlay || !this.welcome_text) return;
        ...
        this.welcome_text.setPosition(width / 2, height / 2).setDepth(1001);
        const assetSize = Math.max(this.welcome_text.width || 1, this.welcome_text.height || 1);
        const targetScale = (Math.min(width, height) * 0.68) / assetSize;
        this.welcome_text.setScale(targetScale);
    }
    ...
    transitionToGame() {
        ...
        this.tweens.add({
            targets: [this.startOverlay, this.welcome_text],
            alpha: 0,
            duration: 260,
            ease: 'Sine.inOut'
        });
    ...
    reflowForResize(gameSize = { width: this.scale.width, height: this.scale.height }) {
        ...
        for (const key in layout) {
            ...
            else if (key.startsWith('fighter_') || key.startsWith('craft_')) this[key].setAngle(0);
